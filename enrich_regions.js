...
const DATA_JS     = path.join(DIR, 'js/data.js');
const RAW_JSON    = path.join(DIR, 'idealista_raw.json');
...
// Форматировать itp/itpInv без лишних нулей, но точно
function fRate(v) {
  // убрать trailing zeros, но сохранить до 4 знаков
...
function buildRegionBlock(r) {
  ...
  const itpInv = r.itpInv !== undefined ? r.itpInv : r.itp;
  ...
  return [
    ...
    `    itp:${fRate(r.itp)}, itpInv:${fRate(itpInv)}, color:'${r.color}', airbnbRestricted:${airbnb}${noteKey},`,
    ...
    `    rentCagr5:${fv(e.rentCagr5)}, rentCagr10:${fv(e.rentCagr10)}, rentCagr20:${fv(e.rentCagr20)}, rentCagrMax:${fv(e.rentCagrMax)}, rentCagrMaxYears:${fv(e.rentCagrMaxYears)},`,
