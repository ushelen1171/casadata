...
function toggleMortgage() {
  const noMortgage = document.getElementById('rc-no-mortgage').checked;
  const fields = document.getElementById('rc-mortgage-fields');
  fields.style.display = noMortgage ? 'none' : 'block';
  if (noMortgage) {
    document.getElementById('rc-down-pct').value = 100;
    updateVal('rc-down-pct', '100%');
  } else {
    document.getElementById('rc-down-pct').value = 20;
    updateVal('rc-down-pct', '20%');
  }
  calcRental();
}

function toggleIncomeSection() {
  const noIncome = document.getElementById('rc-no-income').checked;
  const fields = document.getElementById('rc-income-fields');
  fields.style.display = noIncome ? 'none' : 'block';
  calcRental();
}
...
function calcRental() {
  ...
  const sqm         = +document.getElementById('rc-sqm').value             || 70;
  const noMortgage  = document.getElementById('rc-no-mortgage').checked;
  const downPct     = noMortgage ? 1 : (+document.getElementById('rc-down-pct').value       || 20) / 100;
  const mortgRate   = noMortgage ? 0 : (+document.getElementById('rc-mort-rate').value      || 3.2) / 100;
  ...
  // BLOCK 7: Update renovation cost display
  ...
  const monthlyMortgage = mortgRate > 0 ? loan * (mRate * Math.pow(1+mRate, nPay)) / (Math.pow(1+mRate, nPay) - 1) : 0;
  ...
  if (currentStrategy === 'rent_out') {
    const longRent    = noIncome ? 0 : (+document.getElementById('rc-long-rent').value    || 1200);
    const vacancyPct  = noIncome ? 0 : (+document.getElementById('rc-vacancy').value     || 5) / 100;
    const mgmtPct     = noIncome ? 0 : (+document.getElementById('rc-mgmt-pct').value    || 0) / 100;
    const taxStatus   = noIncome ? 'eu' : (document.getElementById('rc-tax-status').value    || 'eu');
...
function renderCashflowChart(mortgage, income, fixed, years) {
  ...
  let cumCF = 0, deficitYears = 0, breakEvenYear = -1;
  ...
  // Update summary cards before chart
  document.getElementById('rc-deficit-years').textContent = deficitYears.toString();
  document.getElementById('rc-breakeven-year').textContent = breakEvenYear > 0 ? 'Год ' + breakEvenYear : '—';
  document.getElementById('rc-final-cf').textContent = fmtE(cumCF);
...
// ============================================================
// BLOCK 7: Renovation features
// ============================================================

let currentRenovationType = 'light';
const RENO_COSTS = {
  light:    { min: 400, max: 600, types: 'Покраска, полы, ванная' },
  standard: { min: 800, max: 1200, types: 'Кухня + ванная + полы' },
  premium:  { min: 1500, max: 2000, types: 'Все помещения + дизайн' }
};

function toggleRenovationSection(header) {
  const section = document.getElementById('rc-renovation-section');
...
function setRenovationType(type, btn) {
  currentRenovationType = type;
  document.querySelectorAll('#rc-renovation-section .nav-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  updateRenovationCost();
  calcRental();
}

function updateRenovationCost() {
  const sqm = +document.getElementById('rc-sqm').value || 70;
  const duration = +document.getElementById('rc-reno-duration').value || 3;
  const increase = +document.getElementById('rc-reno-increase').value || 8;
  const reno = RENO_COSTS[currentRenovationType];
  const midPrice = (reno.min + reno.max) / 2;
  const totalCost = midPrice * sqm;
  
  // Update renovation cost display
  document.getElementById('rc-reno-cost-per-sqm').textContent = reno.min + '—' + reno.max + ' €/м²';
  document.getElementById('rc-reno-total-cost').textContent = fmtE(totalCost);
  
  // Update slider value displays
  document.getElementById('rc-reno-duration-val').textContent = duration + ' мес';
  document.getElementById('rc-reno-increase-val').textContent = increase + '%';
}
